...
export function assertReportShape(value: unknown): asserts value is SkillUsageReportV1 {
  ...
  if (
    ...
    typeof report.coverage.request_history_complete !== "boolean" ||
    ...
    !Number.isInteger(report.coverage.unresolved_request_candidates) ||
    ...
    !Number.isInteger(report.summary.requests) ||
    !Number.isInteger(report.summary.sessions_with_requests) ||
    !Array.isArray(report.skills) ||
    !Array.isArray(report.activity) ||
    !Array.isArray(report.requested_skills) ||
...
export const reportJsonSchema = {
  ...
  type: "object",
  required: ["schema_version", "tool_version", "query", "coverage", "summary", "activity", "skills", "requested_skills", "zero_load_skills"],
  properties: {
    ...
    coverage: {
      type: "object",
      required: ["history_complete", "request_history_complete", "inventory_status", "files_scanned", "bytes_scanned", "sessions_scanned", "unreadable_records", "unresolved_load_candidates", "unresolved_request_candidates", "unresolved_inventory_sources"],
    },
    summary: { type: "object", required: ["loads", "sessions_with_loads", "requests", "sessions_with_requests"] },
    skills: { type: "array" },
    zero_load_skills: { type: "array" },
    activity: { type: "array" },
    requested_skills: { type: "array" },
  },
} as const;
